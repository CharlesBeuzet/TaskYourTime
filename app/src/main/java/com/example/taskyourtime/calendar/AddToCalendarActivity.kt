...
class AddToCalendarActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        beginDateSetListener = DatePickerDialog.OnDateSetListener { _, year, monthOfYear, dayOfMonth ->
            binding.eventBeginDate.text = String.format(getString(R.string.date_picker_placeholder),dayOfMonth.toString(), (monthOfYear + 1).toString(), year.toString())
        }

        endDateSetListener = DatePickerDialog.OnDateSetListener { _, year, monthOfYear, dayOfMonth ->
            binding.eventEndDate.text = String.format(getString(R.string.date_picker_placeholder),dayOfMonth.toString(), (monthOfYear + 1).toString(), year.toString())
        }

        beginTimeSetListener = TimePickerDialog.OnTimeSetListener { _, hourOfDay, minute ->
            binding.eventBeginTime.text = String.format(getString(R.string.time_picker_placeholder),hourOfDay.toString(), minute.toString())
        }

        endTimeSetListener = TimePickerDialog.OnTimeSetListener { _, hourOfDay, minute ->
            binding.eventEndTime.text = String.format(getString(R.string.time_picker_placeholder),hourOfDay.toString(), minute.toString())
        }
